...
ArmyMedalDialog::ArmyMedalDialog(Army* army, PG_Widget* parent, Rectangle rect)
    :PG_Window(parent, rect, _("medal awarded"), PG_Window::MODAL), d_army(army)
{
    ...
    //the army pic
    d_b_pic = new PG_Button(this, Rectangle(my_width/2 - 25, 40, 50, 50), "",2);
    d_b_pic->EnableReceiver(false);
    d_b_pic->SetIcon(d_army->getPixmap(), 0, 0);

   //the name of the army
    d_l_name = new PG_Label(this, Rectangle(my_width/2 - 100, 100, 200, 20),
                            d_army->getName().c_str());
    d_l_name->SetAlignment(PG_Label::CENTER);

    snprintf(buffer, 30, _("xp: %.2f"), d_army->getXP());
    d_l_xp = new PG_Label(this, Rectangle(my_width/2 - 40, 130, 95, 20), buffer);
    snprintf(buffer, 30, "%i", d_army->getLevel());
    d_l_level1 = new PG_Label(this, Rectangle(my_width/2 - 40, 150, 45, 20),_("level: "));
    d_l_level2 = new PG_Label(this, Rectangle(my_width/2 + 10, 150, 30, 20), buffer);
    d_l_level2->SetFontColor(PG_Color(50, 255, 50));

    d_b_ok = new PG_Button(this, Rectangle(my_width/2 - 40, 190, 80, 30),_("OK"),1);
